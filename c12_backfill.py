...
def normalize_hourly(device_id, rows):
    ...
    for row in rows:
        ...
        # Match the existing c12_master convention:
        # Eastern local timestamps stored without timezone.
        local_time = (
            utc_time
            .astimezone(EASTERN)
            .replace(tzinfo=None)
...
def main():
    if not DB_URL:
        raise RuntimeError(
            "Missing DB_URL or DATABASE_URL."
        )
    ...
    if day_count < 1 or day_count > 31:
        raise RuntimeError(
            "Backfill must cover between "
            "1 and 31 days per run."
        )
    ...
    print(
        "Turner Station C-12 backfill complete. "
