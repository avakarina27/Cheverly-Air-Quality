export default async function handler(req, res) {
  ...
  try {
    if (action === "purpleair_box") {
      ...
      const url =
        `https://api.purpleair.com/v1/sensors?nwlng=${encodeURIComponent(nwlng)}` +
        `&nwlat=${encodeURIComponent(nwlat)}&selng=${encodeURIComponent(selng)}` +
        `&selat=${encodeURIComponent(selat)}&fields=${encodeURIComponent(fields || "")}` +
    ...
    if (action === "quantaq_devices") {
      ...
      const r = await fetch(url, {
        headers: { Authorization: QUANTAQ_KEY },
      });
      const text = await r.text();
      res.status(r.status).send(text);
      return;
